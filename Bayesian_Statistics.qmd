...
data.frame(theta, prior) |>
  ...
  geom_line(color = "#00a68a", size = 1.2) +
...
data.frame(theta, likelihood) |>
  ...
  geom_line(color = "#33bb88", size = 1.2) +
  scale_x_continuous(labels = scales::percent) +
...
posterior_df |>
  ...
  scale_x_continuous(labels = scales::percent) +
  theme_dark_site() +
  scale_color_manual(values = c("#33bb88", "#e05555", "#5599cc"))
```
...
normal_df <- bind_rows(
  ...
  tibble(x = x_normal, density = normalize(dnorm(x_normal, 0, 2.5)), Distribution = "Normal", Variant = "B"),
...
ggplot(df, aes(x = x, y = density)) +
  geom_line(size = 1.1, color = "#00a68a") +
...
Going back to our model, since $\beta_0$ is on the logit scale it can take any value from $-\infty$ to $+\infty$. A sensible prior should reflect that:

1.  it could be negative or positive (log-odds of presence can go either way)

2.  extreme values are unlikely (a very large or very small logit would imply near-certain presence or absence across all sites, which is rarely realistic before seeing the data)

3.  zero is a reasonable central guess (a logit of 0 corresponds to a 50% probability of presence)

These criteria point naturally towards a $Normal$ distribution centred at zero. Following Gelman's recommendation for weakly informative priors on logit-scale parameters, a standard deviation of 2.5 is a sensible default. From the figure above, this corresponds to the middle Normal distribution (Variant B).
...
Which is to say that my guess is that $\beta_0$ will *probably* be somewhere in the distribution of values specified by $Normal(0, 2.5)$.
...
However, if you're ever really stuck and don't have *any* idea of what a sensible prior would be, you can fall back on $Normal(0, 2.5)$ as a weakly informative default. This is again Gelman's recommendation, and it works reasonably well for most real-valued parameters. It looks like this:

```{r}
theta <- seq(-10, 10, length.out = 1000)

prior <- dnorm(theta, 0, 2.5)
...
Which says: I think baby growth rate is probably somewhere near zero, but I'm open to values several units in either direction. It's not completely flat, but it's wide enough not to strongly influence the posterior unless your data are very sparse.
