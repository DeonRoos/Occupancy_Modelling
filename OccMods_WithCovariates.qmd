...
p1 <- ggplot(df) +
  geom_boxplot(aes(x = factor(survey), y = cov),
               fill = "#2d2e30", color = "#cccccc") +
  geom_jitter(aes(x = factor(survey), y = cov),
              width = 0.2, height = 0, color = "#cccccc", alpha = 0.7) +
  labs(x = "Survey", y = "Rainfall") +
  theme_dark_site()

p2 <- ggplot(df) +
  geom_jitter(aes(y = factor(y), x = cov),
              alpha = 0.5, width = 0, height = 0.1, color = "#cccccc") +
...
obs_long$Detection <- factor(obs_long$Detection, levels = c(1, 0), labels = c("Detection", "No detection"))
 ...
 p3 <- ggplot(obs_long, aes(x = Site, y = Survey, fill = Detection)) +
  ...
  scale_fill_manual(values = c("Detection" = "#00a68a", "No detection" = "#2d2e30")) +
...
(p1 + p2 + p3 + plot_annotation(tag_levels = "A", tag_suffix = ")",
    theme = theme(plot.background = element_rect(fill = "#202123", colour = NA))) +
  plot_layout(design = design)) &
  theme(plot.background = element_rect(fill = "#202123", colour = NA),
        panel.background = element_rect(fill = "#202123", colour = NA))
```
...
```{r}
ggplot(df) +
  geom_line(aes(x = rain, y = pred), color = "#00a68a") +
...
p1 <- ggplot(df) +
  geom_histogram(aes(x = temp), fill = "#00a68a", color = "#202123") +
...
p3 <- ggplot(df) +
  geom_histogram(aes(x = tree), fill = "#00a68a", color = "#202123") +
...
p5 <- ggplot(df) +
  geom_histogram(aes(x = cov), fill = "#00a68a", color = "#202123") +
  labs(x = "Rain", y = "Count") +
  theme_dark_site()
...
p1 <- ggplot(fake, aes(x = tree, y = mean.psi)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "#00a68a", alpha = 0.2) +
  geom_line(color = "#00a68a") +
