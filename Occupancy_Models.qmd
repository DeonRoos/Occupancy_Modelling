...
> **Morgan**, in your case this may be; Why are red foxes present at site A but not site B? Is it because site A is closer to urban areas or human settlements? If a site is in a more urbanised landscape, what's the probability red foxes are present? Is that very different from rural sites?
>
> What about features of the habitat? Are red foxes more likely to occupy sites with more woodland edge, or areas with higher prey availability?

> **Stuart**, in your case this may be; Why are bats present at site A but not site B? Is it related to distance to water bodies or riparian habitat? What does the relationship between woodland cover and bat occupancy look like?
>
> What about other features of the landscape? Are bats more likely to occupy sites away from artificial light at night, or where there is more continuous hedgerow connectivity?
...
library(ggplot2)
ggplot(dat) +
  geom_line(aes(x = logit, y = p), color = "#00a68a") +
...
> **Stuart** and **Morgan**, this has two important implications for your fieldwork:
>
> You need to survey each site on multiple occasions, i.e. you need $J$ surveys. If you’re using camera traps (Morgan) or acoustic detectors (Stuart), this means leaving the device in situ for more than one sampling period.
>
...
One practical advantage of the Bayesian approach here is that uncertainty about $z$ flows automatically into uncertainty about $\psi$. If the model is unsure whether a site is truly occupied, that doubt is reflected directly in the width of the credible interval for occupancy probability, without any extra calculation on your part.
...
## Explore: detection bias visualiser

Use the sliders to explore how detection probability and true occupancy interact. Watch the **naive estimate** (the proportion of sites with at least one detection, what a simple GLM would use) diverge from true occupancy as *p* drops. Set *p* = 1.0 to confirm the two converge; then pull it toward 0 and watch the gap open.
...
viewof controls = Inputs.form(
  {
    psi: Inputs.range([0, 1], {value: 0.7, step: 0.01, label: "True occupancy (ψ)"}),
    p:   Inputs.range([0, 1], {value: 0.4, step: 0.01, label: "Detection probability (p)"})
  },
  {
    template: inputs => html`<div style="display:grid;grid-template-columns:1fr 1fr;gap:2px 24px">
      ${inputs.psi}${inputs.p}
    </div>`
  }
)
...
//| echo: false
sim = {
  const rand = mkRng(1988);
  const z = Array.from({length: 30}, () => rand() < controls.psi ? 1 : 0);
  const y = z.map(zi =>
    Array.from({length: 3}, () => rand() < (zi * controls.p) ? 1 : 0)
  );
  const trueOcc  = z.filter(v => v === 1).length / 30;
  const naiveOcc = y.filter(row => row.some(v => v === 1)).length / 30;
  return {z, y, trueOcc, naiveOcc};
}
```
...
//| echo: false
{
  ...
  const barData = [
    {label: "Naive observed", value: sim.naiveOcc, color: "#cc6677"},
    {label: "Truly occupied", value: sim.trueOcc,  color: "#4db8a4"},
    {label: "ψ (slider)",     value: psi,          color: "#00a68a"}
  ];
  ...
  const bar = Plot.plot({
    ...
    marginLeft: 130,
    marginRight: 46,
    style: {background: "transparent", color: "#cccccc", fontSize: "12px"},
    x: {
      domain: [0, 1],
    ...
    y: {label: null, domain: ["Naive observed", "Truly occupied", "ψ (slider)"]},
    marks: [
      Plot.barX(barData, {x: "value", y: "label", fill: d => d.color, tip: true}),
      Plot.text(barData, {
        x: "value", y: "label",
        text: d => ` ${(d.value * 100).toFixed(0)}%`,
        textAnchor: "start", fill: "#cccccc", dx: 4
      }),
      Plot.ruleX([0], {stroke: "#444"})
    ]
  });

  const colLabels = ["True z", ...Array.from({length: J}, (_, j) => `Survey ${j + 1}`)];
  const mData     = sim.z.flatMap((zi, i) => [
    {site: i + 1, col: 0,     val: zi,         type: "state"},
    ...sim.y[i].map((yij, j) => ({site: i + 1, col: j + 1, val: yij, type: "det"}))
  ]);
  ...
  const matrix = Plot.plot({
    ...
    height: N * 10 + 42,
    caption: null,
    style: {background: "transparent", color: "#cccccc", fontSize: "11px"},
    x: {
      domain: Array.from({length: J + 1}, (_, i) => i),
      tickFormat: i => colLabels[i],
      label: "Survey"
    },
    y: {
    ...
    color: {
      legend: true,
      domain: ["Absent", "Detected", "Present (z)"],
      range:  ["#2d2e30", "#4db8a4", "#c9a84c"],
      style:  {background: "transparent", color: "#ccc"}
    },
    marks: [
      Plot.cell(mData, {
        x: "col", y: "site",
        fill: d => d.val === 0 ? "Absent" : d.type === "state" ? "Present (z)" : "Detected",
        stroke: "#1a1b1e", strokeWidth: 1, tip: true
      })
    ]
  });

  return html`<div>
    <div style="margin-bottom:16px">${bar}</div>
    <div>${matrix}</div>
  </div>`;
}
```
