...
Let's see what that might look like for our ten sites in the figure above:
...
This is something called a *variance co-variance matrix*. Each row and column represents a site (in this case); $S1$ is Site 1, $S2$ is Site 2, and so on. The values, between 0 and 1, represent the correlation between $S_i$ and $S_I$. What you might notice is that Site 1 is perfectly correlated with itself. That's good. It should be. Your height is perfectly correlated with *your height*. What's more interesting is when we look at different sites. For example, $S1$ and $S2$ have a correlation of 0.42. $S1$ and $S3$ is 0.63. $S2$ and $S3$ is 0.39. This shows that sites 1 and 3 are very similar to each other.
...
Our question is then - Is that because they are close to each other in space? Is that why they're so similar?

This is where we need something called *pairwise* distance. Pairwise distance is simply the distance between a pair of sites. As simple as that.

If we have both pieces of this information, we can estimate spatial autocorrelation, which may look like:
...
The figure above shows how the similarity between nearby locations changes as the pairwise distance between them increases. This is often called spatial autocorrelation, the idea that places close to each other tend to be more alike than places far apart. On the x-axis is the pairwise distance between two locations, how far apart they are. On the y-axis is the correlation between what we measure at those locations, like whether a species is present or not.
